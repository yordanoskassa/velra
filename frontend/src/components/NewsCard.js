  ...
  return (
    <View style={styles.subscriptionPromptContainer}>
      ...
      <Text style={styles.subscriptionPromptText}>
        Subscribe to velra PRO to unlock multiple AI-powered market insights and premium features.
      </Text>
...
const NewsCard = ({ article, onPress, index = 0, isSavedScreen = false, onSaveToggle = null }) => {
      ...
      if (!insights && !error) {
        ...
        if (!isSubscribed) {
          ...
          if (showPaywall) {
            ...
            // Use the real velra PRO paywall
  ...
  const getImageSource = () => {
    if (safeArticle.imageUrl) {
      return { uri: safeArticle.imageUrl };
    }
          ...
          {safeArticle.imageUrl && !insightLoaded ? (
            <Card.Cover 
              ...
              onError={(error) => {
                ...
                // Set insightLoaded to true to show placeholder instead
                setInsightLoaded(true);
              }}
            />
          ) : (
            <View style={[styles.placeholderImage, { backgroundColor: `${getCategoryColor()}15` }]}>
              ...
              <View style={styles.logoContainer}>
                ...
                <Text style={styles.decoderText}>velra</Text>
              </View>
            </View>
          )}
        </TouchableOpacity>
