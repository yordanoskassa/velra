...
const CustomPaywall = ({ 
  ...
  const glowColor = glowAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ['rgba(255, 215, 0, 0.1)', 'rgba(255, 215, 0, 0.5)']
  });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.contentWrapper}>
        ...
        {/* Header - Only DECODER text, logo moved closer to notch */}
        <Motion.View {...getEntranceAnimation(0.1)} style={styles.headerContainer}>
          <View style={styles.titleContainer}>
            <Text style={styles.titleDecoder}>DECODR</Text>
            <Animated.View 
              style={[
                styles.proContainer,
        ...
        <View style={styles.featuresContainer}>
          {renderFeatureItem('flash', 'Unlimited market insights', 0)}
          {renderFeatureItem('bookmark', 'Save headlines for later', 1)}
          {renderFeatureItem('star', 'Weekly expert headline picks', 2)}
          {renderFeatureItem('trending-up', 'Priority access to new features', 3)}
        </View>
...
const styles = StyleSheet.create({
  ...
  proContainer: {
    backgroundColor: '#FFD700', // Yellow background for PRO
  ...
  titlePro: {
    fontSize: 18,
    fontWeight: 'bold',
    color: '#000000', // Black text for PRO
  },
  subtitle: {
    fontSize: 18,
    color: '#333333',
  ...
  featureIconContainer: {
    ...
    height: 32,
    borderRadius: 16,
    backgroundColor: 'rgba(242, 242, 242, 0.9)', // Light gray background instead of yellow
    justifyContent: 'center',
    alignItems: 'center',
    marginRight: 12,
  },
